...
type
  TEEGHdr = packed record //Next: EDF header
   ...
   ID: array [1..66] of char; //80 Patient ID
   RESERVED : array [1..10] of byte;
   MEDICAL,WEIGHTKG,HEIGHTCM,DEMOGRAPHIC: byte;
   RECORDID: array [1..64] of char; //64ascii : local patient identification
   LATLONGALT : array [1..4] of longword;
   STARTDATETTIME: array [1..2] of longword;
   BIRTHDATETIME: array [1..2] of longword;
   HDRLENGTH: word;
   ICD: array [1..6] of char;//ICD classification
   MANUFACTURER: UInt64;
   RESERVED2: array [1..6] of char;
   HEADSIZEMM: array [1..3] of word;
   REFPOS: array [1..3] of single;
   GROUNDPOS: array [1..3] of single;
   RECORDS: int64;  //8 ascii : number of data records (-1 if unknown, obey item 10 of the additional EDF+ specs)
   DURATIONSECNUM,DURATIONSECDENOM: longword;  //8 ascii : duration of a data record, in seconds
   NS: word;   //4 ascii : number of signals (ns) in data record
   RESERVED3: word;  
 ...
 TChanHdr = packed record //Next: Poly5 header
    ...
    dimension: array [1..6] of char;
    dimensioncode: word;
    physicalmin,physicalmax:double;
    digitmin,digitmax:double;
    Res : array [1..68] of char;
    lowpass,highpass,notch: single;
    samplespersec,typeofdata: longword;
    SensorPos: array [1..3] of single;
    SensorInfo : array [1..20] of char;
    {Impedance: byte;
...
begin
  ...
  samplesec := 888;//strtofloat(Trim(EEGHdr.DURATIONSEC));
