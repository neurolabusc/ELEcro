...
interface
uses eeg_type, sysutils,dialogs, DateUtils,IniFiles, StrUtils,deb,classes;
...
function VHDR4VMRK (lVMRKFilename: string): string;//given VMRK, returns suggested Datafile


implementation

function VHDR4VMRK (lVMRKFilename: string): string;//given VMRK, returns suggested VHDR
var
  lV: TVMRK;

begin

    result := changefileext(lVMRKFilename,'.vhdr');
    if Fileexists (result) then
      exit;
    LoadVMRK(lVMRKFilename, lV);

    result := extractfiledir(lVMRKFilename)+pathdelim+lV.Datafile;
    if Fileexists (result) then
      exit;

    result := changefileext(lV.Datafile,'.vhdr');
    if Fileexists (result) then
      exit;

    //result := VHDR4VMRK; //failed to find suitable VHDR
end;
